...
const repositorio_2 = require("../src/modulos/envios/repositorio");
const entorno_1 = require("../src/configuracion/entorno");
const repoPedidosMock = repositorio_1.repositorioPedidos;
const repoEnviosMock = repositorio_2.repositorioEnvios;
...
const pedidoAceptado = {
    ...
    clienteId: 'u-cli-1',
    estado: client_1.EstadoPedido.ACEPTADO,
    ...
    lineas: [{ id: 'lin-1', cantidad: 1, precioUnitario: '34.90', estadoLinea: client_1.EstadoPedido.ACEPTADO, disenadorId: 'u-dis-1', variante: { talla: 'M', color: 'Natural', sku: 'CLI-M', producto: { nombre: 'Camiseta', slug: 'camiseta' } }, disenador: { nombreMarca: 'Liñares Moda' } }],
    envio: { id: 'env-1', transportista: client_1.Transportista.CORREOS_VERDE, envioEcologico: true, numeroSeguimiento: null, entregaEstimada: null, fechaEnvio: null, fechaEntrega: null },
};
const envioSimulado = {
    id: 'env-1',
    pedidoId: 'ped-1',
    transportista: client_1.Transportista.CORREOS_VERDE,
    envioEcologico: true,
...
describe('GET /pedidos/:pedidoId/envio', () => {
    it('401 sin autenticación', async () => {
        ...
        const r = await (0, supertest_1.default)(app).get('/pedidos/ped-1/envio');
        expect(r.status).toBe(401);
    });
    it('200 cliente ve el envío de su pedido', async () => {
        repoPedidosMock.buscarPorId.mockResolvedValueOnce(pedidoAceptado);
        repoEnviosMock.buscarDePedido.mockResolvedValueOnce(envioSimulado);
        const app = (0, aplicacion_1.crearAplicacion)();
...
//# sourceMappingURL=envios.rutas.test.js.map